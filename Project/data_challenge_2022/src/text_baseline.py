...
data_path = '../data/'
# Create a graph
G = nx.read_edgelist(data_path+'edgelist.txt', delimiter=',',
...
for i, node_pair in enumerate(node_pairs):
    ...
    X_test[i, 1] = abs(len(abstracts[node_pair[0]]) -
                       len(abstracts[node_pair[1]]))
    X_test[i, 2] = len(
        abstracts[node_pair[0]].intersection(abstracts[node_pair[1]]))
